...
public class TfraisrRepositoryTests extends EntityTest {
    ...
    @Test
    public void test_M_sumAllTiersByVcKey() {
        LawfirmEntity lawfirm = createLawfirm("MYLAW");
        TDossiers dossier = createDossier(lawfirm, EnumVCOwner.OWNER_VC);
        ...
        BigDecimal sumAllTiersByVcKey = tFraisRepository.sumAllTiersByVcKey(dossier.getIdDoss(), lawfirm.getVckey());

        assertNotNull(sumAllTiersByVcKey);
        // could be different
        assertEquals(tFrais.getMontant().toBigInteger(), sumAllTiersByVcKey.toBigInteger());
    }

    @Test
    public void test_N_findAllWithPagination_withDossier() {
        LawfirmEntity lawfirm = createLawfirm("MYLAW");
        TDossiers dossier = createDossier(lawfirm, EnumVCOwner.OWNER_VC);
        ...
        Pageable pageable = new OffsetBasedPageRequest(10, 10, Sort.by(order, order2));

        Page<TFrais> allCompta = tFraisRepository.findAllWithPagination(tFrais.getVcKey(), "", "", "%", "%", "%", pageable);


        assertNotNull(allCompta);
        // could be different
        assertEquals(allCompta.getTotalElements(), 1);
    }

    @Test
    public void test_O_findAllWithPagination_withoutDossier() {
        LawfirmEntity lawfirm = createLawfirm("MYLAW");
        TDossiers dossier = createDossier(lawfirm, EnumVCOwner.OWNER_VC);

        TFrais tFrais = createTFrais(lawfirm, dossier);
        tFrais.setTDossiers(null);
        tFrais.setIdDoss(null);
        testEntityManager.persist(tFrais);
