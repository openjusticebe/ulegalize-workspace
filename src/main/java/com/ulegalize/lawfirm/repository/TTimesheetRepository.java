...
public interface TTimesheetRepository extends JpaRepository<TTimesheet, Long>, JpaSpecificationExecutor<TTimesheet> {
    ...
    @Query(
            value = "SELECT t from TTimesheet t " +
                    ...
                    " and d.nomenclature like COALESCE(concat('%', ?2, '%'), '%')" +
                    " and tsT.idTs like COALESCE(?3, '%')",
            countQuery = "SELECT count(t) from TTimesheet t" +
                    ...
                    " and d.nomenclature like COALESCE(concat('%', ?2, '%'), '%')" +
                    " and tsT.idTs like COALESCE(?3, '%')")
    Page<TTimesheet> findAllWithPagination(Long vcUserIds, String searchNomenclature, Integer idTsType, Pageable pageable);
    ...
    @Query(nativeQuery = true, value = "select" +
            ...
            " timesheet.id_gest as idGest, users.email as email, timesheet.ts_type as tsType, timesheetType.description as tsTypeDescription, " +
            ...
            "         inner join t_users users on users.id = timesheet.id_gest" +
