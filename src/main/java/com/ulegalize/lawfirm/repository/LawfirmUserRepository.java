...
public interface LawfirmUserRepository extends CrudRepository<LawfirmUsers, Long> {
    ...
    @Query(value = "SELECT l from LawfirmUsers l where l.lawfirm.vckey = :vcKey and l.isActive = true and not exists (select dr.vcOwner from TDossierRights dr where dr.vcUserId = l.id and dr.dossierId = :dossierId)")
